...
- 日期：2026-09-13
- 对象：**PR #15**（分支 `launcher-extended`）——基于上游 **v1.0.7**（`846c44c1`），**193 个提交 / 157 文件 / +37,821 / −7,115**，**不含**上游 v1.0.8 ~ v1.1.2 的提交。
...
1. **两条线已分叉**：本分支相对 v1.0.7 是 157 文件 / +37,821 / −7,115（含 2026-09-13 追加的 UI 统一整轮与行尾统一，见 §1b）；上游自己从 v1.0.7 到 v1.1.2 走了 99 文件 / +18,833 / −722。**41 个文件双方都改过** —— 这是冲突面的来源，也是 GitHub 显示 *can't automatically merge* 的原因。
...
|---|---|---|
| `src/DshLauncher/Services/`（业务逻辑） | 87 文件，+19,120 / −431 | 36 文件，+8,513 / −345 |
| `src/DshLauncher/` 其余（窗口 XAML / 代码后置 / 入口等） | 43 文件，+14,307 / −1,939 | 25 文件，+6,286 / −298 |
| `Models/` | 14 文件，+819 / −17 | 17 文件，+917 / −7 |
| `tests/` | 2 文件，+2,354 / −4,616 | 13 文件，+2,641 / −43 |
| `docs/` | 7 文件，+978 / −0（含本报告自身 +131） | 0 |
| 其它（README / .gitignore / CI 等） | 5 文件，+374 / −112 | 8 文件，+476 / −29 |
| **合计** | **158 文件（新增 105 / 删除 1），+37,952 / −7,115**（含本报告自身） | **99 文件，+18,833 / −722** |
