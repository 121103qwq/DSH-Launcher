...
> **状态：已完成（2026-09-13）。** 十个主题 A–J 共 **13 个变更集（127–139）** 已落地并发布（work-log 118–131）；仍未做的项在文中保留 `[ ]` 并注明原因/前置条件。本文件转为**历史清单 + 遗留索引**。
>
...
- [x] **各页滚动位置记忆待补**（2026-09-13 复核）：目前**只有**设置页与扩展页（`UiStateStore`）；对话 / 日志中心 / 任务 / 插件矩阵切走再切回会回到顶部——补法明确（`UiStateStore` + `ScrollViewer.ScrollChanged`），留后续小变更集（**2026-09-13 完成，变更集 146**：共享 `ScrollMemory` + `ui-state.json` 通用字段，日志页实测"播种 600 → 打开即 6.2%"）
...
- [x] 提示（`ShowNotice`）时长统一（**2026-09-13 完成，变更集 144**）：自适应 `4s + 字数/30`（上限 12s）+ 错误类常驻 + 悬停暂停（按剩余时间继续）+ 手动关闭 `×`；实测 103 字文案 7.0s 消失（公式值 7s）
- [x] 列表 `Delete` 快捷键（**2026-09-13 完成，变更集 144**）：用户口径＝**只加扩展页已安装列表**；等价于点删除按钮（复用同一流程与二次确认）；守门＝输入框内不抢键 / 未选中 / 按钮不可用时不动
...
| 2026-09-13 | **K 收尾**：整轮发布（dist = 安装副本 MD5 `b2fca822…`）+ 发布后安装副本实测 + 整轮验收报告（work-log/131） |
| 2026-09-13 | **变更集 140（验收修复）**：修「导入实例」菜单窄白框（`MenuItem` 子元素≠`Header`，内容未渲染）、危险按钮悬停改**只加深描边**（`DangerStrongBrush`）、标题栏三字形 18→**22px**；各补一条门禁 |
| 2026-09-13 | **变更集 141（验收第二轮修正）**：危险按钮悬停**改回"红底白字"**（140 做反了）；标题栏 `×` 按**墨迹实测**改 26px 与 `□` 等大（门禁口径改为"按墨迹等大"）；门禁不再用正则 |
| 2026-09-13 | **变更集 142**：修「删除版本」悬停红底红字（内联 `Foreground` 覆盖样式触发器）；门禁升级为「危险按钮不得内联 Foreground/Background」；截图工具新增 **hover 步骤/模式** 实测悬停态 |
| 2026-09-13 | **变更集 143**：设置→「插件与技能来源」删除按钮（代码创建）套 DangerButton；门禁补 C# 侧盲区（又抓出第二处）；Skill 市场卡片统一为插件市场卡片形态；**修 Skill 市场列表横向溢出**（长描述撑破条目 → 按钮跑出窗口），两个市场列表条目绑定宽度到列表；验收截图改归档到 `screenshots/<轮次>/` |
| 2026-09-13 | **变更集 144**：提示条按文字长度自适应消失（4s+字数/30，上限 12s；错误类常驻；悬停暂停；`×` 手动关闭）+ 扩展列表 `Delete` 快捷键（复用删除流程） |
| 2026-09-13 | **变更集 145**：三态分色收尾 —— 把 `ExtensionWindow.SetStatusText` 提为共享 `Services/StatusTextStyler`，四页（Conversation / VersionControl / LogCenter / LauncherTask）接入，版本列表补空态；失败文案补「下一步点哪里」 |
| 2026-09-13 | **变更集 146**：滚动位置记忆扩展到内嵌页 —— `ui-state.json` 通用字段 `ScrollOffsets` + 共享 `Services/ScrollMemory`（防抖写盘/恢复）；对话/任务/日志/设置分类接入；修两处真实缺陷：设置页恢复需等下一轮布局、日志页要靠具名 `LogScroll` 而非"视觉树第一个" |
