...
- 支持导入、导出 `.dshpack` 整合包；导入时创建新版本，不覆盖已有版本。
...
- 只保存 API Key 的环境变量名称，不在 Launcher 配置中保存密钥明文。
- Provider 自动同步只在双方都开启同步且实例已停止时生效；没有 llm Provider 配置时不会覆盖文件。
...
当前源码版本为 **v0.1.11**。下载、变更说明和 SHA-256 信息请查看 [GitHub Releases](https://github.com/121103qwq/DSH-Launcher/releases)。
