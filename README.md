...
| 92 | `src/DshLauncher/Services/DangerousConfigAuditService.cs`（新）+ 设置页安全体检分区扩展（+7 断言） | **#20 增量 5：危险配置检查（work-log/74）**：按用户勾选范围检查权限档位/`DSH_PERMISSION_MODE`/`approval.policy`/遥测模式与导出口/依赖来源/凭据文件保护/HMR；**只直接读文件，不调 `dsh --dump-config`**（调研发现它会写盘）；读不出的 `!!js` 表达式记 Notes 不猜；安全样本零发现 + 危险样本全命中（双向自校准）+ 纯读取反证 |
...
| 90 | `Services/DshPackFormat.cs` / `DshPackArchive.cs` / `DshPackImportService.cs` / `DshPackFileDownloader.cs` / `DshPackWriter.cs` / `PackExportService.cs`（均新）+ 实例设置导出按钮 + `docs/DSH_CONTRACT_INVENTORY.md` | **#24 整合包规范对齐（work-log/70、72）**：容器识别（`.dspack` v2/v3、旧 `.tgz`）+ manifest **v2–v5** 解析与校验；导入到新建实例（权威重建 `package.json`、`overrides/`、`home/`，失败整体回滚）；`files[]` 下载（默认拒绝、显式同意、sha256+size 双校验、镜像回退）；导出 **manifest v4 + `.dspack` v2**（自家 v1 保留为旧格式）；SelfTest +40 条（含往返） |
